...
func (s *CacheTestSuite) initPostgres(ctx context.Context) (*db.Pool, error) {
	...
	// Create a new pool
	pool, err := db.NewPool(ctx, fmt.Sprintf("user=%s password=%s dbname=%s sslmode=disable", dbUser, dbPassword, dbName))
	if err != nil {
		return nil, err
	}
	return pool, nil
...
func (s *CacheTestSuite) TestCacheString() {
	...
	val, ok := s.GetWithContext(ctx, cacheKey)
	assert.True(s.T(), ok)
	assert.Equal(s.T(), value, val.(string))
}
...
func (s *CacheTestSuite) TestCacheStructData() {
	...
	val, ok := s.GetWithContext(ctx, cacheKey)
	assert.True(s.T(), ok)
	assert.Equal(s.T(), value, val.(User))
}
