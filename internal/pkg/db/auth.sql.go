...
const cleanupExpiredStates = `-- name: CleanupExpiredStates :exec
DELETE FROM auth_oauth_states WHERE expires_at < NOW()
`

func (q *Queries) CleanupExpiredStates(ctx context.Context, db DBTX) error {
	_, err := db.Exec(ctx, cleanupExpiredStates)
	return err
}
...
func (q *Queries) CreateOAuthState(ctx context.Context, db DBTX, arg CreateOAuthStateParams) error {
	_, err := db.Exec(ctx, createOAuthState,
		arg.State,
		arg.Provider,
		arg.RedirectUrl,
		arg.ExpiresAt,
...
func (q *Queries) DeleteOAuthState(ctx context.Context, db DBTX, state string) error {
	_, err := db.Exec(ctx, deleteOAuthState, state)
	return err
}
...
SELECT state, provider, redirect_url, created_at, expires_at FROM auth_oauth_states WHERE state = $1
`

func (q *Queries) GetOAuthState(ctx context.Context, db DBTX, state string) (AuthOauthState, error) {
	row := db.QueryRow(ctx, getOAuthState, state)
