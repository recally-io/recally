...
function BUFFER(input) {
  ...
  if (typeof input === "string") return new Buffer(input).pointer;
...
function fromColor(n) {
  if (n === 1) return [libmupdf.HEAPF32[_wasm_color]];
  if (n === 3)
    return [
...
function fromPoint(ptr) {
  let addr = ptr >> 2;
  return [libmupdf.HEAPF32[addr + 0], libmupdf.HEAPF32[addr + 1]];
}
...
export class Buffer extends Userdata {
  constructor(arg) {
    if (typeof arg === "undefined") super(libmupdf._wasm_new_buffer(1024));
...
export class ColorSpace extends Userdata {
  constructor(from, name) {
    if (typeof from === "number") super(from);
    else
  ...
  getType() {
    return (
      ColorSpace.COLORSPACE_TYPES[
        libmupdf._wasm_colorspace_get_type(this.pointer)
      ] || "None"
    );
  }
...
export class Font extends Userdata {
  constructor(name_or_pointer, data, subfont = 0) {
    ...
    if (typeof name_or_pointer === "number") {
      ...
      else pointer = libmupdf._wasm_new_base14_font(STRING(name_or_pointer));
    }
    super(pointer);
  }
  getName() {
...
Font.SIMPLE_ENCODING = ["Latin", "Greek", "Cyrillic"];
Font.ADOBE_CNS = 0;
Font.ADOBE_GB = 1;
Font.ADOBE_JAPAN = 2;
Font.ADOBE_KOREA = 3;
...
export class Image extends Userdata {
  ...
  getMask() {
    ...
    if (mask) return new Image(libmupdf._wasm_keep_image(mask));
    return null;
  }
  toPixmap() {
...
export class StrokeState extends Userdata {
  constructor(pointer) {
    if (typeof pointer === "number") super(pointer);
    else super(libmupdf._wasm_new_stroke_state());
  }
  getLineCap() {
...
StrokeState.LINE_CAP = ["Butt", "Round", "Square", "Triangle"];
StrokeState.LINE_JOIN = ["Miter", "Round", "Bevel", "MiterXPS"];
export class Path extends Userdata {
  constructor(pointer) {
    if (typeof pointer === "number") super(pointer);
    else super(libmupdf._wasm_new_path());
  }
  getBounds(strokeState, transform) {
    if (strokeState !== null) checkType(strokeState, StrokeState);
...
export class Text extends Userdata {
  constructor(pointer) {
    if (typeof pointer === "number") super(pointer);
    else super(libmupdf._wasm_new_text());
  }
  getBounds(strokeState, transform) {
    if (strokeState !== null) checkType(strokeState, StrokeState);
    checkMatrix(transform);
...
export class Pixmap extends Userdata {
  ...
  clear(value) {
    if (typeof value === "undefined") libmupdf._wasm_clear_pixmap(this.pointer);
    else libmupdf._wasm_clear_pixmap_with_value(this.pointer, value);
  }
  getWidth() {
  ...
  asJPEG(quality, invert_cmyk) {
    let buf = libmupdf._wasm_new_buffer_from_pixmap_as_jpeg(
      this.pointer,
      quality,
      invert_cmyk,
    );
  ...
  tint(black, white) {
    ...
    if (typeof black === "number") black_hex = black;
    else if (black instanceof Array && black.length === 3)
      black_hex =
        ((black[0] * 255) << 16) | ((black[1] * 255) << 8) | (black[2] * 255);
    if (typeof white === "number") white_hex = white;
    else if (white instanceof Array && white.length === 3)
      white =
        ((white[0] * 255) << 16) | ((white[1] * 255) << 8) | (white[2] * 255);
  ...
  warp(points, width, height) {
    ...
    return new Pixmap(
      libmupdf._wasm_warp_pixmap(this.pointer, QUAD(quad), width, height),
    );
  }
}
...
export class StructuredText extends Userdata {
  ...
  highlight(p, q, max_hits = 100) {
    ...
    try {
      hits = Malloc(32 * max_hits);
      let n = libmupdf._wasm_highlight_selection(
        this.pointer,
        POINT(p),
        POINT2(q),
        hits,
      ...
      for (let i = 0; i < n; ++i) result.push(fromQuad(hits + i * 32));
    } finally {
      Free(hits);
    }
    return result;
...
export class Document extends Userdata {
  static openDocument(from, magic) {
    ...
    if (pdf) return new PDFDocument(pdf);
  ...
  resolveLinkDestination(link) {
    ...
    else dest = libmupdf._wasm_resolve_link_dest(this.pointer, STRING(link));
    return {
  ...
  outlineIterator() {
    return new OutlineIterator(
      libmupdf._wasm_new_outline_iterator(this.pointer),
    );
  }
}
...
export class OutlineIterator extends Userdata {
  ...
  insert(item) {
    return libmupdf._wasm_outline_iterator_insert(
      this.pointer,
...
export class Page extends Userdata {
  ...
  toStructuredText(options = "") {
    checkType(options, "string");
    return new StructuredText(
      libmupdf._wasm_new_stext_page_from_page(this.pointer, STRING(options)),
    );
  }
  getLinks() {
    let links = [];
  ...
  createLink(bbox, uri) {
    checkRect(bbox);
    return new Link(
      libmupdf._wasm_create_link(this.pointer, RECT(bbox), STRING(uri)),
    );
  }
  deleteLink(link) {
    checkType(link, Link);
...
export class PDFDocument extends Document {
  constructor(arg1) {
    ...
    else if (typeof arg1 === "number") super(arg1);
    else {
      let doc = Document.openDocument(arg1, "application/pdf");
      if (doc instanceof PDFDocument) return doc;
  ...
  _toPDFObject(obj) {
    ...
    if (obj === null || obj === undefined) return this.newNull();
    ...
    if (typeof obj === "number") {
      if (obj === (obj | 0)) return this.newInteger(obj);
      return this.newReal(obj);
    }
    if (typeof obj === "boolean") return this.newBoolean(obj);
    if (obj instanceof Array) {
      let result = this.newArray(obj.length);
      for (let item of obj) result.push(item);
      return result;
    }
    if (obj instanceof Object) {
      let result = this.newDictionary();
      for (let key in obj) result.put(key, obj[key]);
      return result;
    }
  ...
  createObject() {
    ...
    return this._fromPDFObjectNew(
      libmupdf._wasm_pdf_new_indirect(this.pointer, num),
    );
  }
  newNull() {
  ...
  newIndirect(v) {
    return this._fromPDFObjectNew(
      libmupdf._wasm_pdf_new_indirect(this.pointer, v),
    );
  }
  newArray(cap = 8) {
    return this._fromPDFObjectNew(
      libmupdf._wasm_pdf_new_array(this.pointer, cap),
    );
  }
  newDictionary(cap = 8) {
    return this._fromPDFObjectNew(
      libmupdf._wasm_pdf_new_dict(this.pointer, cap),
    );
  }
  deleteObject(num) {
  ...
  newGraftMap() {
    return new PDFGraftMap(
      this,
      libmupdf._wasm_pdf_new_graft_map(this.pointer),
    );
  }
  graftObject(obj) {
    checkType(obj, PDFObject);
    return this._fromPDFObjectNew(
      libmupdf._wasm_pdf_graft_object(this.pointer, obj.pointer),
    );
  }
  ...
  findPage(index) {
    ...
    return this._fromPDFObjectKeep(
  ...
  saveToBuffer(options = "") {
    ...
    return new Buffer(
      libmupdf._wasm_pdf_write_document_buffer(this.pointer, STRING(options)),
    );
  }
...
export class PDFPage extends Page {
  ...
  getObject() {
    return this._doc._fromPDFObjectKeep(
...
export class PDFObject extends Userdata {
  ...
  _get(path) {
    ...
    for (let key of path) {
      if (typeof key === "number") obj = libmupdf._wasm_pdf_array_get(obj, key);
  ...
  getInheritable(key) {
    ...
    return this._doc._fromPDFObjectKeep(
      libmupdf._wasm_pdf_dict_gets_inheritable(this.pointer, STRING(key)),
    );
  }
  put(key, value) {
    ...
    else libmupdf._wasm_pdf_dict_puts(this.pointer, STRING(key), value.pointer);
    return value;
  }
  push(value) {
    value = this._doc._toPDFObject(value);
  ...
  delete(key) {
    ...
    else libmupdf._wasm_pdf_dict_dels(this.pointer, STRING(key));
  }
  valueOf() {
  ...
  asJS(seen) {
    if (this.isIndirect()) {
      ...
      if (!seen) return `${ref} 0 R`;
      if (ref in seen) return seen[ref];
...
export class PDFGraftMap extends Userdata {
  ...
  graftObject(obj) {
    checkType(obj, PDFObject);
    return this._doc._fromPDFObjectNew(
...
export class PDFAnnotation extends Userdata {
  ...
  getObject() {
    return this._doc._fromPDFObjectKeep(
      libmupdf._wasm_pdf_annot_obj(this.pointer),
    );
  }
  getBounds() {
  ...
  setLineEndingStyles(start, end) {
    ...
    return libmupdf._wasm_pdf_set_annot_line_ending_styles(
      this.pointer,
      start_ix,
      end_ix,
    );
  ...
  getInteriorColor() {
    return fromColor(
      libmupdf._wasm_pdf_annot_interior_color(this.pointer, COLOR()),
    );
  }
  setColor(color) {
    checkColor(color);
  ...
  getBorderStyle() {
    return (
      PDFAnnotation.BORDER_STYLE[
        libmupdf._wasm_pdf_annot_border_style(this.pointer)
      ] || "Solid"
    );
  }
  ...
  getBorderEffect() {
    return (
      PDFAnnotation.BORDER_EFFECT[
        libmupdf._wasm_pdf_annot_border_effect(this.pointer)
      ] || "None"
    );
  }
  ...
  getIntent() {
    return (
      PDFAnnotation.INTENT[libmupdf._wasm_pdf_annot_intent(this.pointer)] ||
      null
    );
  }
  setIntent(value) {
  ...
  getDefaultAppearance() {
    ...
    let color = fromColor(
      libmupdf._wasm_pdf_annot_default_appearance_color(this.pointer, COLOR()),
    );
  ...
  getFieldType() {
    return (
      PDFWidget.WIDGET_TYPES[
        libmupdf._wasm_pdf_annot_field_type(this.pointer)
      ] || "button"
    );
  }
  isButton() {
